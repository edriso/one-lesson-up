<template>
  <div class="space-y-2">
    ...
    <div v-if="showAutoDetection" class="p-3 bg-blue-50 border border-blue-200 rounded-md">
      <div class="flex items-start space-x-3">
        ...
        <div class="flex-1">
          <h3 class="text-sm font-medium text-blue-800">We detected your timezone</h3>
    ...
    <div v-else>
        ...
        <SelectContent class="max-h-80">
          ...
          <div class="max-h-60 overflow-y-auto">
              ...
              <div v-if="group.timezones.length > 0" class="py-1">
                ...
                >
                  <div class="flex flex-col">
                    ...
                    <span class="text-sm text-muted-foreground">{{ timezone.city }}, {{ timezone.country }}</span>
                  </div>
            ...
            <!-- No results -->
            <div v-if="filteredTimezones.length === 0" class="p-4 text-center text-muted-foreground">
      ...
      <div v-if="selectedTimezone && currentTime" class="mt-2 text-sm text-muted-foreground">
      ...
      <p v-else class="text-xs text-muted-foreground mt-1">
        Your timezone affects when you can earn time bonuses for completing lessons. 
        <strong>Note:</strong> You can only update your timezone once every 30 days.
      </p>
    </div>
...
const daysUntilCanUpdate = computed(() => {
  if (!props.timezoneUpdatedAt) return 0
  const daysSinceUpdate = Math.floor((new Date().getTime() - new Date(props.timezoneUpdatedAt).getTime()) / (1000 * 60 * 60 * 24))
...
const filteredGroups = computed(() => {
  ...
  return groups.map(groupName => ({
    name: groupName,
    timezones: filteredTimezones.value.filter(tz => tz.group === groupName)
  }))
})
