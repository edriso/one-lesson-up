...
const loadMoreActivities = async () => {
    ...
    try {
        const response = await fetch(
            `/api/activities/load-more?page=${currentPage.value}`,
        );
...
const isCourseComplete = computed(() => {
    ...
    return props.user.current_class.modules.every(
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div
            class="flex h-full flex-1 flex-col gap-6 bg-gradient-to-br from-primary/5 to-secondary/5"
        >
            <!-- Welcome Section -->
            <div class="mb-4">
                ...
                <div class="flex items-center gap-4 text-muted-foreground">
                    ...
                    <span
                        >{{ recent_activities.length }} activities this
                        week</span
                    >
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                <div class="space-y-6 lg:col-span-2">
                    ...
                    <Card
                        ...
                        class="border-primary/20 shadow-sm transition-shadow hover:shadow-md"
                    >
                        <CardHeader>
                            <CardTitle class="flex items-center gap-2">
                                ...
                                Current Class:
                                <span
                                    class="cursor-pointer text-primary underline hover:text-primary/80"
                                    @click="
                                        router.visit(
                                            `/classes/${user.current_class.id}`,
                                        )
                                    "
                                >
                        ...
                        <CardContent class="space-y-6">
                            <div v-if="currentModule" class="space-y-3">
                                <div class="flex items-center justify-between">
                                    ...
                                    >
                                        {{
                                            currentModule.completion_percentage
                                        }}% Complete
                                    </Badge>
                                </div>
                                <Progress
                    ...
                    <Card class="shadow-sm">
                        ...
                        <CardContent>
                            ...
                            <div v-else class="space-y-3">
                                <div
                                    v-for="lesson in upcoming_lessons"
                                    :key="lesson.id"
                                    class="flex flex-wrap items-center justify-between gap-y-4 rounded-lg border p-4 transition-all hover:shadow-md"
                                    :class="[
                                        lesson.completed
                            ...
                            <div class="space-y-4">
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        ...
                                        >
                                            {{
                                                last_completed_lesson?.module_title
                                            }}
                                            •
                                            {{
                                                last_completed_lesson?.class_title
                                            }}
                                        </p>
                                        <p
                                        ...
                                        <div
                                            v-if="last_completed_lesson?.link"
                                            class="mt-3"
                                        >
                                            <a
                                                :href="
                                                    last_completed_lesson.link
                                                "
                                                target="_blank"
                                ...
                                >
                                    <span class="text-xs text-muted-foreground">
                                        ...
                                        {{
                                            formatShortDateTime(
                                                last_completed_lesson?.completed_at ||
                                                    '',
                                            )
                                        }}
                                    </span>
                        ...
                        <CardContent>
                            ...
                            >
                                <div
                                    ...
                                    :key="activity.id"
                                    class="flex items-start gap-3 rounded-lg bg-muted/30 p-3 transition-colors hover:bg-muted/50"
                                >
                                    <component
                                        ...
                                        >
                                            ...
                                            >
                                                <Link
                                                    ...
                                                    class="-m-1 flex items-center gap-2 rounded-md p-1 transition-colors hover:bg-muted/50"
                                                >
                                                    <UserInfo
                                                        :user="{
                                                            id: activity.user
                                            ...
                                            >
                                                ...
                                                <span
                                                    v-if="
                                                        ...
                                                        0
                                                    "
                                                    class="rounded bg-secondary/20 px-1.5 py-0.5 text-xs font-medium text-secondary-foreground"
                                                >
                                                    +{{
                                                        activity.points_earned
                                                    }}
                                ...
                                <div v-else class="flex justify-center py-4">
                                    <span class="text-sm text-muted-foreground"
                                        >No more activities to load</span
                                    >
                                </div>
                            </div>
                        </CardContent>
